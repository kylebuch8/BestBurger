(function() {
	...
	angular.module(Best.appName).controller('ProfileCtrl', ['$scope', '$http', function($scope, $http) {
		...
		if (localStorage.getItem(Best.authProvider) === 'google') {
			console.log('authProvider is google');
			console.log('token is: ' + localStorage.getItem(Best.tokenString));

			var token = localStorage.getItem(Best.tokenString),
		...
		if (localStorage.getItem(Best.authProvider) === 'facebook') {
			var token = localStorage.getItem(Best.tokenString),
